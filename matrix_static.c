...
void multiplyStatic(int A[SIZE][SIZE], int B[SIZE][SIZE], int C[SIZE][SIZE], int threads) {
    int i, j, k; // Loop variables

    // Parallel multiplication using OpenMP with static scheduling
    #pragma omp parallel for num_threads(threads) schedule(static, CHUNK_SIZE) private(i, j, k) shared(A, B, C)
    for (i = 0; i < SIZE; i++) {  
        for (j = 0; j < SIZE; j++) {  
            C[i][j] = 0;  
            for (k = 0; k < SIZE; k++) {  
...
int main() {
    ...
    printf("\n==== Running with Static Scheduling ====\n");
    for (int t = 0; t < 3; t++) {
        ...
        printf("Average time with %d threads (Static): %f sec\n", threads, avgTime);
    }
